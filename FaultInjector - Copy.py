...
def update_vehicle_status(vehicle):
    while(connected):
        ...
        updateText += ('\nBattery Capacity: %s MAH' % vehicle.parameters['BATT_CAPACITY'] +
                       ...
                       '\nWind Pitch: %s'             % vehicle.parameters['SIM_WIND_DIR_Z'] + 
...
def update_mission_readout(new_selected=-1):
    ...
    if connected:
        ...
        for x, cmd in enumerate(cmds):
            mission_readout.insert(END,str(x) + ': ' + command_key_to_name(cmd.command) + ', ' +str(cmd.x) + ', ' + str(cmd.y) + ', ' + str(cmd.z) + '\n')
            command_list.append(cmd)     
        if new_selected != -1:
...
def delete_wp(selected_index):
    ...
    mission_custom.pop(selected_index)
    waypoint_info = []
    update_mission_readout()
    send_mission()
    print('Waypoint ' + str(selected_index) + ' deleted.')
...
def vary_wp(selected, mean, stddev=-1):
    ...
    mission_custom[selected].x += mag*math.cos(math.radians(theta)) #actually latitude
    mission_custom[selected].y += mag*math.sin(math.radians(theta)) #actually longitude
    print('Waypoint ' + str(selected) + ' changed to ' + str((mission_custom[selected].x, mission_custom[selected].y)))
    send_mission()
    update_mission_readout()
...
def change_wp_loc(selected, x_change, y_change):
    ...
    if not waypoints_been_modified:
        mission_custom = mission_actual[:]
        waypoints_been_modified = True 
    old_x = mission_custom[selected].x
    old_y = mission_custom[selected].y
    lat_change,long_change = miles_to_coords(x_change, y_change, mission_custom[selected].y)
    print((lat_change, long_change))
    mission_custom[selected].x += long_change
    mission_custom[selected].y += lat_change
    
    print('Waypoint ' + str(selected) + ': ' + str((old_x, old_y)) + ' changed to ' + str((mission_custom[selected].x, mission_custom[selected].y)))
    send_mission()
    update_mission_readout()
...
def set_wp(selected, lat, lon):
    if connected:
        ...
        if not waypoints_been_modified:
            mission_custom = mission_actual[:]
            waypoints_been_modified = True
        mission_custom[selected].x = lat
        mission_custom[selected].y = lon
        send_mission()
        update_mission_readout()
        print('Waypoint ' + str(selected+1) + ' set to ' + str((mission_custom[selected].x, mission_custom[selected].y)))
...
def edit_flyby_wp(selected, radius):
    ...
    if not found:
        #if no entries into wp_info, make a new one and start the listener thread
        new_wp_info(selected, radius=radius)
        thread.start_new_thread(waypoint_listener, ())
    print( 'WP_RADIUS set to ' + str(radius) + ' for wp #' + str(selected))
...
def repo_up_wp(selected):
    ...
    if selected == 0:
        ...
        return
    temp = mission_custom[selected-1]
    mission_custom[selected-1] = mission_custom[selected]
    mission_custom[selected]   = temp
    if waypoint_info:
        for wp in waypoint_info:
            if wp[0] == selected:
                wp[0] -= 1
    send_mission()
    update_mission_readout(selected-1)
    print('Waypoint ' + str(selected+1) + ' Moved up')
...
def repo_down_wp(selected):
    ...
    if not waypoints_been_modified:
        mission_custom = mission_actual[:]
        waypoints_been_modified = True
    if selected == len(mission_custom)-1:
...
def swap_wps(wp1, wp2):
    ...
    if -1 < wp1 <= len(mission_custom) and -1 < wp2 <= len(mission_custom):
        #do the swapping
        temp = mission_custom[wp1]
        mission_custom[wp1] = mission_custom[wp2]
        mission_custom[wp2] = temp
        #fix wp_info
        if waypoint_info:
            for wp in waypoint_info:
                if wp[0] == wp1:
                ...
                if wp[0] == wp2:
                    wp[0] = wp1
        print('Waypoint ' + str(wp1+1) + ' swapped with' + str(wp2+1))
...
def send_wind_params():
    ...
    while connected:
        ...
        vehicle.parameters['SIM_WIND_DIR_Z'] = float(pitch)
...
def reset():
    ...
    vehicle.parameters['SIM_WIND_DIR_Z'] = 0
            ...
            if int(command[0]) <= t:
                ...
                if type(command[1]) != type('str'):
                    ...
                    if   command[1] == 'TURBULENCE'      or command[1] == 'TURB':
                        ...
                        repo_up_wp(int(command[2])+1)
                    elif command[1] == 'WP_MOVE_DOWN' or command[1] == 'DOWN':
                        repo_down_wp(int(command[2])+1)
                    elif command[1] == 'WP_SWAP' or command[1] == 'SWAP':
                        swap_wps(int(command[2])+1, int(command[3])+1)
                    elif command[1] == 'WP_SKIP' or command[1] == 'SKIP':
                        skip_wp(int(command[2])+1)
                    elif command[1] == 'WP_RADIUS' or command[1] == 'RAD':
                        edit_flyby_wp(int(command[2])+1, int(command[3]))
                    elif command[1] == 'WP_OFFSET' or command[1] == 'OFFSET':
                        change_wp_loc(int(command[2])+1, int(command[3]), int(command[4]))
